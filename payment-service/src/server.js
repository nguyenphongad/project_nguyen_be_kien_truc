...
// Thêm endpoint để decode token mà không verify signature (giúp debug)
app.get('/debug-token', (req, res) => {
  ...
  try {
    ...
    const { debugToken } = require('./middleware/authMiddleware');
    const decodedInfo = debugToken(token);
    
    return res.status(200).json({
      success: true,
      message: 'Token đã được decode (không verify signature)',
      decodedInfo: decodedInfo,
      jwtSecretUsed: process.env.JWT_SECRET ? '[HIDDEN]' : 'Not set in .env (using default)',
      note: 'Kiểm tra log server để xem chi tiết hơn'
    });
  } catch (error) {
    return res.status(500).json({
      success: false,
